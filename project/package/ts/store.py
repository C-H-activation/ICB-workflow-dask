# dags/project/package/store/store.py
...
def writeTS(n: int, name:str, path=os.getcwd()):
    """Write out transition-state structure depending on input."""
    mol = getTS(n)
    mol.writeMolecule(name=name, path=path)


def getTS(n: int):
    """Return transistion-state structure depending on input."""
    ...
    func = switcher.get(n, lambda: "Invalid transistion-state structure requested.")
    return func()


def ts1():
    """Create template with d(B(72)-Ir(19)-C(24)-H(27)) = 320 degrees.
    Values in parenthesis are indices in xmol file."""
